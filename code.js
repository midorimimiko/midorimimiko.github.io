...
function _rch() { return CHARS[Math.floor(Math.random() * CHARS.length)]; }

function _startGlitchLoop(el) {
    const texts     = [el.dataset.a, el.dataset.b];
    const displayMs = parseInt(el.dataset.display) || 3000;
    const glitchMs  = parseInt(el.dataset.glitch)  || 1500;
    let idx = 0, glitching = false;
    el.textContent = texts[0];

    function glitchTo(from, to, done) {
        glitching = true;
        const start = Date.now();
        function frame() {
            const p = Math.min((Date.now() - start) / glitchMs, 1);
            if (p >= 1) {
                el.textContent = to;
            ...
            for (let i = 0; i < len; i++) {
                ...
                const resolved = p > 0.55 && i < to.length && Math.random() < lp * (len / (i + 1)) * 0.8;
                out += resolved ? to[i] : _rch();
            }
            el.textContent = out;
            if (Math.random() < 0.4) {
                const ox = (Math.random() - 0.5) * 6;
                el.style.textShadow = `${ox + 2}px 0 rgba(255,20,60,.8), ${ox - 2}px 0 rgba(0,255,210,.8)`;
                el.style.transform  = `translateX(${(Math.random() - .5) * 8}px)`;
            } else {
                el.style.textShadow = el.style.transform = '';
            }
    ...
    function next() {
        const from = texts[idx];
        idx = (idx + 1) % texts.length;
        glitchTo(from, texts[idx], () => {
            _glitchTimers.push(setTimeout(next, displayMs));
        });
    }
    _glitchTimers.push(setTimeout(next, displayMs));
}

function _stopAllGlitch() {
    _glitchTimers.forEach(clearTimeout);
    _glitchTimers.length = 0;
}

function _initGlitchEls() {
    document.querySelectorAll('.glitch-el').forEach(_startGlitchLoop);
}
// ─────────────────────────────────────────────────────────────
...
巨大的衝擊力讓衣櫃裡的木質層板發出不堪重負的吱呀聲，散發著淡淡香氛的洋裝與塵埃如落花般覆蓋在艾格雷瓦身上
...
「哦？妳說惡魔不能傷害人類，又說可以折磨，那到底是怎樣？」艾格雷瓦看向窗外，天空是一片異樣的紅色，令人分不清究竟是白天還是黑夜，鄰近之處似乎沒有其他建築

娜塔莎急急忙忙把艾格雷瓦拉離窗邊，重新將被自己撞飛的窗簾裝上，「我幹嘛跟你解釋那麼多？反正我已經警告過你了，我半夜還得回博物館處理那堆該死的雕像，在我回來前，你最好乖乖待著」

「那我的吃食跟換洗衣物怎麼辦？」艾格雷瓦像個講不聽的小孩將窗簾的一角掀起，被娜塔莎狠狠拍開
...
「這段期間你就穿這些吧！可能對你來說不太合身，不過你一個寄人籬下吃軟飯的傢伙沒有挑剔的資格，何況這些衣服的料子都比你身上那套好太多了！」娜塔莎打開箱子，裡面是整齊摺疊的男裝

儘管艾格雷瓦對服裝品味沒有什麼概念，但在蘿絲父母的耳濡目染下他還是能夠看得出，那些衣裝雖是設計內斂，但布料的質地與精緻的做工就如娜塔莎所說，比常人所穿的要好得多
...
「為什麼是我睡地板？我就想睡床上」艾格雷瓦豪不客氣地一屁股坐在娜塔莎的床上

「這是我的家！」娜塔莎氣急敗壞的拽起艾格雷瓦的領子

艾格雷瓦挑眉：「是妳家又怎樣？偶爾換換口味睡地板也不錯啊，不然我就出去看看有沒有好心人願意收留我……」

娜塔莎咬牙切齒，權衡輕重後還是只能氣憤的坐到地鋪上：「只有今天讓你睡床！」

「如果妳的床睡起來不舒服那我自然會還給妳，晚安！」
...
此外，艾格雷瓦的社交圈娜塔莎想像的還要貧瘠，除了幾個標註為「打工」或「外送」的號碼，頻繁聯絡的只有寥寥幾個：「伯父」、「伯母」，以及唯一擁有名字的聯絡人──「蘿絲」
...
眼看時間也差不多，娜塔莎將手機原封不動地放回原位，接著打開自己的手機，在備忘錄記下剛才獲得的重大情報後便動身去處理原本未完成的工作`
        ...
        {
            title: "Chapter 3",
            content: `夜色低垂，舊城區的街道被昏黃的街燈拉出長長的黑影

艾格雷瓦與娜塔莎站在博物館後門的陰影處，這是艾格雷瓦被「綁架」後的第五天，被退貨回來的他此刻身上的是被綁架那天所穿的服裝

雖然他曾覺得娜塔莎哥哥的那些衣服布料確實舒服得多，但這身粗糙的棉質觸感反而讓他有一種腳踏實地的真實感

「好了，物歸原位」娜塔莎壓低聲音，有些神經質地環顧四周，「你最好閉緊嘴巴，要是讓我在人界聽到什麼『黑色翅膀』的傳言，我發誓我絕對會──」

話音未落，那扇鏽跡斑斑的鐵門發出「嘎吱」一聲，被從內推開

「艾格？」

剛加完班的蘿絲提著沉重的書袋，眉眼間夾著些許疲憊，她的眼下帶著淡淡的青紫色，顯然這幾天都沒睡好

在看清艾格雷瓦身影的瞬間，她手裡的書袋直接掉在了地上，發出沉悶的撞擊聲

「艾格！你這幾天跑哪去了！」蘿絲三步併作兩步衝上來，雙手緊緊抓住艾格雷瓦的肩膀，「你終於回來了……我這幾天找你找得快瘋了！你沒事吧？你去哪裡了？這女孩是？」

艾格雷瓦還未回答，身旁的娜塔莎便搶先開口：「我是那個、對了！我是他媽那邊的親戚，前幾天臨時抓了他來我們家幫忙處理些急事，然後，呃，不巧他那天手機沒帶在身上……真的很不好意思讓妳擔心了，那我先走囉，哦呵呵！」

娜塔莎用極為鱉腳的語氣說出扯得要命的謊言，艾格雷瓦本想吐槽「妳要不要聽聽看自己在說什麼？」，但娜塔莎溜得實在太快，艾格雷瓦只能朝她的背影翻了個大白眼

然而，蘿絲對此卻沒產生半分懷疑，甚至露出了鬆口氣的笑容：「原來是這樣啊……你沒事就好，不過真沒想到你跟你母親那邊的親戚還有聯繫啊，我還以為……」

雖然這是個能讓蘿絲安心的說詞，可蘿絲就這樣輕易相信陌生人讓艾格雷瓦感到不快，於是他非常不給面子的拆台：「我是被那丫頭綁走的，她是惡魔，妳沒看她耳朵還是尖的？」

聞言，蘿絲驚訝的張大了眼，隨即忍不住笑了出來：「看來你這幾天處理的事情相當麻煩吧？不過把人家女孩子說成惡魔還是有點過火了，那身哥特風格的cosplay很可愛啊！」

顯然蘿絲把「惡魔」當成了艾格雷瓦式的惡劣玩笑，艾格雷瓦有些氣悶，明明娜塔莎的謊言如此拙劣，自己卻沒有足夠證據能夠直接揭穿對方身份，嚥不下這口氣的他只能用另一個事實來反駁：「反正她不是我的親戚，我跟生下我的那兩個人早就沒有關係了，妳應該很清楚才對」

可惜這也只是讓蘿絲往其他錯誤的方向過度解讀，她溫柔地拍了拍艾格雷瓦的肩膀，眼神裡帶著一種「這孩子真不坦率」的慈愛：「但你心底其實還是放不下，所以才會去幫忙的對吧？因為你這孩子就是這樣刀子嘴豆腐心啊」

艾格雷瓦乾脆閉上嘴，放棄這場無意義的交流

「對了，就在你被抓去幫忙的那晚，博物館發生了大事！」蘿絲一邊走一邊興奮地在包裡翻找，「那些原本應該在整裝區的雕像竟然全部發生了異變！而且我偷偷跟你說，我還撿到了──」

「八成就是娜塔莎幹的好事吧，我那天就是撞見她在那些雕像前鬼鬼祟祟的才被她綁架」艾格雷瓦面無表情地插話

蘿絲停下腳步，腦細胞飛快地運轉，試圖將艾格雷瓦的話轉化為她能理解的邏輯：「你的意思是……剛才那個女孩子叫做娜塔莎，然後她家是做雕刻相關的行業，找你處理的急事也跟這個有關？啊！是因為發生了雕像損壞的意外所以人手不足需要幫忙？這樣說的話那些雕像只是暫代修復的備品？可是館長看起來不知情還慌慌張張的……」

「夠了！妳為什麼就那麼相信她的鬼話啊？跟妳說過了不是那樣！」艾格雷瓦受不了地揉著自己緊縮的眉頭，他覺得與其說蘿絲是單純，不如說是死腦筋的一旦認定了某些事情就堅信不移

先入為主的觀念使人們只能看見自己想看見的東西，一旦大腦預設了某種神祕學的立場，連古人隨手寫下的創作都能被當成是埋沒於時間洪流中的歷史碎片
...
艾格雷瓦原本以為這場荒誕的鬧劇會隨著他回到家而終止，然而他顯然低估了惡魔的執著`
        },
        {
            title: "Chapter 4",
            content: `咪`
